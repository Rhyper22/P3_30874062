...
const transporter = nodemailer.createTransport({
  service:'Gmail',
  auth:{
    user:'rufax22@gmail.com',
    pass:'blitdjcdnegaomkt'
  }
});
...
//-------------------------------------------
    ...
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
      ...
      description: 'Reloj',
      image:'https://images.pexels.com/photos/1682821/pexels-photo-1682821.jpeg?auto=compress&cs=tinysrgb&w=400',
      }
      });
...
function registroUsuariosPost(req,res){
 ...
 const mensaje = {
  from:'rufax22@gmail.com',
  to:email,
  subject:'!Bienvenido queridisimo usuario¡',
  text:`Hola ${nombre} ${apellido}, !estamos aqui para brindarte la mejor experiencia en relojes inteligentes¡`
  }
...
db.run(sql,[nombre_de_usuario,apellido,cedula,telefono,cliente_id,producto_id,cantidad,total_pagado,ip_cliente,codigo,numeroT,cvv,mesV,year,currency,descripcion],(err)=>{
  if(err) return console.error(err.message);
  const mailOptions = {
    from: 'rufax22@gmail.com', // Reemplaza con tu dirección de correo electrónico
...
function mostrarPaginaProductos(req,res){
...
// Generar un token único
 ...
 res.cookie('securityToken',token, { httpOnly: true, secure: true });
  ...
  const recoveryURL = `https://webfunesrafael.onrender.com/restablecer?token=${token}`;
